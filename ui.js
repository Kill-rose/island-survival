...
function resetMessageState() {
  ...
  if (textZone && textZone.elt) {
    textZone.elt.removeEventListener('click', onMessageClick);
  }
  if (actionPanel && actionPanel.elt && _panelSkipHandler) {
    actionPanel.elt.removeEventListener('click', _panelSkipHandler, true);
    _panelSkipHandler = null;
  }
  messageWaiting = false;
...
function showOpening() {
  ...
  function titleBtn(label, fn, accent) {
    ...
    b.addEventListener('click', () => { playSfx('select'); startBgm(); }, { once: true });
  ...
  if (localStorage.getItem('savedata')) {
    titleBtn('📂  通常セーブからロード', () => loadAndRefresh('savedata'), false);
  }
  if (localStorage.getItem('autosavedata')) {
    titleBtn('↻  オートセーブからロード', () => loadAndRefresh('autosavedata'), false);
  }

  titleBtn('📖  遊び方', () => showTitleSubPage('howtoplay'), false);
  titleBtn('⚔  攻略',   () => showTitleSubPage('guide'), false);
  titleBtn('©  クレジット', () => showTitleSubPage('credits'), false);
...
function showTitleSubPage(type) {
  ...
  wrap.elt.style.cssText = 'width:100%;max-width:640px;flex:0 0 auto;padding:clamp(16px,4vw,40px);';

  if (type === 'credits') {
    wrap.elt.innerHTML = `
      <h1 style="font-family:Cinzel,serif;color:#c9a84c;font-size:clamp(18px,3vw,28px);margin:0 0 20px;border-bottom:1px solid #3a4235;padding-bottom:8px;">クレジット</h1>
      <p style="color:#7a8572;line-height:1.8;font-size:clamp(12px,1.6vw,15px);margin:0 0 24px;">
        『無人島サバイバル』で使用している素材の提供元です。背景の一部にAI生成画像を使用しています。
      </p>
      <section style="border-top:1px solid #3a4235;padding:18px 0;">
        <h2 style="color:#8fbc5a;font-size:clamp(15px,2.2vw,20px);margin:0 0 12px;">画像素材</h2>
        <ul style="color:#d4d8cc;line-height:2;font-size:clamp(13px,1.8vw,17px);padding-left:22px;margin:0;">
          <li><a href="https://torihasi.blog.fc2.com/" target="_blank" rel="noopener noreferrer" style="color:#a8d888;">モンスターフリー素材置き場</a></li>
          <li><a href="https://uzi-material.com/" target="_blank" rel="noopener noreferrer" style="color:#a8d888;">素材屋『氏』</a></li>
          <li><a href="https://www.pakutaso.com/" target="_blank" rel="noopener noreferrer" style="color:#a8d888;">フリー素材ぱくたそ</a></li>
          <li><a href="https://game-materials.com/" target="_blank" rel="noopener noreferrer" style="color:#a8d888;">ゲームまてりあるず</a></li>
        </ul>
      </section>
      <section style="border-top:1px solid #3a4235;padding:18px 0;">
        <h2 style="color:#8fbc5a;font-size:clamp(15px,2.2vw,20px);margin:0 0 12px;">音楽・効果音</h2>
        <ul style="color:#d4d8cc;line-height:2;font-size:clamp(13px,1.8vw,17px);padding-left:22px;margin:0;">
          <li><a href="https://soundeffect-lab.info/" target="_blank" rel="noopener noreferrer" style="color:#a8d888;">効果音ラボ</a></li>
          <li><a href="https://maou.audio/" target="_blank" rel="noopener noreferrer" style="color:#a8d888;">魔王魂</a></li>
          <li><a href="https://oyu-bgm.com/" target="_blank" rel="noopener noreferrer" style="color:#a8d888;">おゆひよこ</a></li>
          <li><a href="https://peritune.com/" target="_blank" rel="noopener noreferrer" style="color:#a8d888;">PeriTune</a></li>
        </ul>
      </section>
      <p style="color:#4a5245;line-height:1.8;font-size:clamp(11px,1.4vw,14px);margin:8px 0 0;">
        素材の利用規約・クレジット表記は、各提供元の案内をご確認ください。
      </p>
    `;
    ...
    wrap.elt.innerHTML = `
      ...
      <h2 style="color:#8fbc5a;font-size:clamp(14px,2.2vw,20px);margin:16px 0 8px;">困ったときのヒント</h2>
      <p style="color:#d4d8cc;line-height:1.8;font-size:clamp(13px,1.8vw,17px);">
        アイテムを選んで<b style="color:#a8d888">「説明」</b>を確認しましょう。<br>
        コマンドにマウスオーバーすると、その操作の説明が表示されます。<br>
        セリフや資料にも、次に進むための手がかりがあります。
      </p>
      <h2 style="color:#8fbc5a;font-size:clamp(14px,2.2vw,20px);margin:16px 0 8px;">時間について</h2>
      <p style="color:#d4d8cc;line-height:1.8;font-size:clamp(13px,1.8vw,17px);">
        行動するたびに時間が経過します。時間によってイベントや状況が変わります。<br>
        体力や気力が最大値の4分の1以下になると赤く表示され、探索や待機の結果に「疲れてきた。」と表示されます。<br>
        疲れたら、食料や焚火を活用して立て直しましょう。<br><br>
    ...
    wrap.elt.innerHTML = `
      ...
      <p style="color:#d4d8cc;line-height:1.8;font-size:clamp(13px,1.8vw,17px);">
        ...
        探索中の戦闘で敵を倒すと、次の「前進」に必要な撃破数が減る。<br>
        深い層ほど強い敵と貴重な素材が出現する。無理に進まず、戻って準備するのも有効。
      </p>
      <h2 style="color:#8fbc5a;font-size:clamp(14px,2.2vw,20px);margin:16px 0 8px;">クラフトと準備</h2>
      <p style="color:#d4d8cc;line-height:1.8;font-size:clamp(13px,1.8vw,17px);">
        序盤は草から紐を作り、木材を集めて道具を整える。<br>
        遠距離武器は距離を保てるため、射程の長い敵との戦いで役立つ。<br>
        回復アイテムは戦闘中にも使える。食料を使い切らず、強敵に備えて残しておく。
      </p>
      ...
      <p style="color:#d4d8cc;line-height:1.8;font-size:clamp(13px,1.8vw,17px);">
        ...
        「観察」ボタンで戦況のヒントが得られる。<br>
        通常の敵は射程外なら攻撃できない。後退で距離を取り、届く武器を選ぶ。
      </p>
      <h2 style="color:#8fbc5a;font-size:clamp(14px,2.2vw,20px);margin:16px 0 8px;">研究所</h2>
      <p style="color:#d4d8cc;line-height:1.8;font-size:clamp(13px,1.8vw,17px);">
        洞窟の深部には研究所へつながる場所がある。<br>
        研究所では部屋ごとに見つかるものが違い、資料や端末が手がかりになる。<br>
        廊下のイベント後に、他の部屋を詳しく調べられるようになる。
      </p>
  ...
  let topBackBtn = document.createElement('button');
  topBackBtn.textContent = '← タイトルに戻る';
  topBackBtn.style.cssText = 'margin-bottom:18px;font-size:clamp(13px,2vw,18px);padding:10px 28px;background:transparent;border:1px solid #3a4235;color:#7a8572;cursor:pointer;font-family:Noto Serif JP,serif;';
  topBackBtn.addEventListener('click', () => showOpening());
  wrap.elt.insertBefore(topBackBtn, wrap.elt.firstChild);
...
function showHowToPlay() {
  const pages = [
    ...
    // ページ4：注意点
    `<b>【遊び方 4/4：注意点】</b><br><br>
    状況に応じて、アイテムや資料を確認しながら進めましょう。<br><br>
...
function updateBgImage(place) {
  ...
  const timeOfDay = ((elapsedTime % 24) + 24) % 24;
  const plazaImage = timeOfDay >= 21 || timeOfDay < 6
    ? 'plaza_night.jpg'
    : timeOfDay >= 18
      ? 'plaza_even.jpg'
      : 'plaza.jpg';
  const forestImage = timeOfDay >= 21 || timeOfDay < 6
    ? 'forest_night.jpg'
    : timeOfDay >= 18
      ? 'forest_even.jpg'
      : 'forest.jpg';
  const caveImage = timeOfDay >= 21 || timeOfDay < 6
    ? 'cave_night.jpg'
    : timeOfDay >= 18
      ? 'cave_even.jpg'
      : 'cave.jpg';
  const bgMap = {
    ...
    '廊下':   'lab_corridor.png',
    '研究室': 'lab_room.png',
    '実験室': 'lab_experiment.png',
    '倉庫':   'lab_storage.png',
  };
  let fname = bgMap[place] || 'plaza.jpg';
  leftTop.style('background-image', `url('${fname}')`);
  leftTop.style('background-size', 'cover');
  leftTop.style('background-position', 'center');

  if (battle.active) {
    if (battle.isSeaBoss || battle.isNaoto) playBgm('boss');
    else if (['廊下', '研究室', '実験室', '倉庫'].includes(place)) playBgm('labBattle');
    else if (place === '洞窟') playBgm('caveBattle');
    else playBgm('forestBattle');
  } else if (place === '広場') playBgm('plaza');
  else if (place === '森') playBgm('forest');
  else if (place === '洞窟') playBgm('cave');
  else playBgm('lab');
}
...
function updateParams(force = false) {
  ...
  let hpColor = player.hp <= MAX_HP / 4 ? 'red' : '#eee';
  let mpColor = player.mp <= MAX_MP / 4 ? 'red' : '#eee';
  ...
  createDiv(`
    ...
    <div class="param" style="color:${hpColor}">体力: ${player.hp} / ${MAX_HP}</div>
    <div class="param" style="color:${mpColor}">気力: ${player.mp} / ${MAX_MP}</div>
    ${fuelStr}
  `).parent(rightWindow);
  ...
  if (Object.keys(itemCounts).length === 0) {
    ...
    for (let name in itemCounts) {
      ...
      div.elt.addEventListener('click', (function(n) {
        return function() { selectedItem = n; playSfx('select'); updateParams(); };
      })(name));
    }
  }
  ...
  if (battle.active) {
    let labEnemyNames = ['ウミウシ（変異体）', '魚（変異体）', 'タコ（変異体）'];
  ...
  function addVolumeSlider(label, value, onInput) {
    let volumeDiv = createDiv().parent(rightWindow);
    volumeDiv.elt.style.cssText = 'display:flex;align-items:center;gap:8px;margin-bottom:4px;';
    let volumeLabel = createDiv(label).parent(volumeDiv);
    volumeLabel.elt.style.cssText = 'color:var(--text-dim);font-size:clamp(10px,1.3vw,13px);flex:0 0 auto;min-width:42px;';
    let volumeSlider = createElement('input').parent(volumeDiv);
    volumeSlider.attribute('type', 'range');
    volumeSlider.attribute('min', '0');
    volumeSlider.attribute('max', '1');
    volumeSlider.attribute('step', '0.01');
    volumeSlider.elt.value = value;
    volumeSlider.elt.style.cssText = 'flex:1;accent-color:var(--accent);cursor:pointer;';
    let volumeValue = createDiv(`${Math.round(value * 100)}%`).parent(volumeDiv);
    volumeValue.elt.style.cssText = 'color:var(--accent);font-size:clamp(10px,1.3vw,13px);min-width:34px;text-align:right;';
    volumeSlider.elt.addEventListener('input', () => {
      onInput(volumeSlider.elt.value);
      volumeValue.elt.textContent = `${Math.round(Number(volumeSlider.elt.value) * 100)}%`;
    });
  }
  addVolumeSlider('BGM', bgmVolume, setBgmVolume);
  addVolumeSlider('SE', seVolume, setSeVolume);

  let saveDiv = createDiv().id('saveButtons').parent(rightWindow);
  if (!battle.active) {
    createButton('通常セーブ').parent(saveDiv).mousePressed(() => saveGame());
  }
  createButton('ログ').parent(saveDiv).mousePressed(() => showBacklog());
  let btnLoad = createButton('ロード').parent(saveDiv);
  if (!localStorage.getItem('savedata') && !localStorage.getItem('autosavedata')) btnLoad.attribute('disabled', 'true');
  btnLoad.mousePressed(() => showLoadMenu());
