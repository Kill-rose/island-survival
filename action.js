...
function withFatigueNotice(message) {
  if (player.hp <= MAX_HP / 4 || player.mp <= MAX_MP / 4) {
    return `${message}<br>疲れてきた。`;
  }
  return message;
}
...
function showMainActions() {
  ...
  let monsterImage = leftTop.elt.querySelector('.monster-image');
  if (monsterImage) monsterImage.remove();
  ...
  if (currentPlace === '広場' && keiState === 'plaza' && !keiPlazaArrived
    ...
    showMessage(
      ...
      '足を引きずりながらも、なんとかたどり着いたらしい。',
      true,
      () => showKeiMessage(
        'ケイ：足、だいぶよくなりました。あなたのおかげです。',
        true,
        () => showKeiMessage(
          'ケイ：この島のこと、一緒に調べてみませんか？',
          true,
  ...
  if (currentPlace === '広場') {
    foundDocuments.forEach(([documentName, label]) => {
      if (!documentObtained[documentName]) return;
      let documentButton = createButton(label).parent(actionPanel);
      documentButton.mousePressed(() => {
        let content = documentContents[documentName] || '内容を読み取れない。';
        showMessage(content, true, () => showMainActions());
      });
    });
  }
  ...
  if (currentPlace === '森' || currentPlace === '洞窟') {
    ...
    // 序盤は探索5回、または探索中に敵を倒した時点で前進できる
    let exploreLimit = curLayer <= 3 ? 5 : EXPLORE_LIMIT;
    if (!atMax && (cd > 0 || cnt >= exploreLimit || placeExploreKillBonus[currentPlace])) {
...
function onRaftClick() {
  ...
  if (keiPlazaArrived) {
    showPortrait('kei', 'default');
    showKeiMessage(
      'ケイ：待ってください。このいかだ、２人で乗るには無理があると思いませんか？<br>' +
      'それに無謀すぎます。目的地もないのに……<br>' +
      'もっと確実な方法を探しませんか？',
      true,
      () => { hidePortrait(); showMainActions(); }
    );
    return;
  }
  if (player.hp < 80 || player.mp < 30) {
    showMessage('体調が万全になってから出発しよう。');
    return;
  }
  // 出発演出→海上ボス戦
  showMessage(
...
function showMoveOptions() {
  ...
  places.forEach(place => {
    ...
    createButton(place).attribute('data-sfx', 'none').parent(actionPanel)
      .mousePressed(() => { playSfx('move'); moveTo(place); });
  });
  ...
  if (labUnlocked) {
    ...
    rooms.forEach(room => {
      ...
      btn.attribute('data-sfx', 'none');
      btn.mousePressed(() => { playSfx('move'); moveToLab(room); });
    });
    // 洞窟6層への戻り口
    if (currentRoom !== null) {
      createButton('洞窟（6層）').attribute('data-sfx', 'none').parent(actionPanel).mousePressed(() => {
        playSfx('move');
        currentRoom = null;
        moveTo('洞窟');
      });
...
function explore(place) {
  if (exploreCooldown[place] > 0) {
    showMessage(withFatigueNotice(`ここからはこれ以上何も見つからなさそうだ。<br>時間を置いてみよう。`));
    return;
  }
  ...
  let exploreLimit = (place === '森' || place === '洞窟') && layer <= 3 ? 5 : EXPLORE_LIMIT;
  if (exploreCounts[place] >= exploreLimit) {
    ...
    if (elapsedTime < 150 && !itemCounts['トランシーバー'] && !cave6TransceiverLost) {
      ...
      if (cave6Count <= 1) {
        ...
        showMessage(
          ...
          '<span style="color:var(--accent)">（使用すると助けを呼べるかもしれない）</span>',
          true, () => { updateElapsedTime(); updateParams(); showMainActions(); }
        );
        updateParams();
        return;
  ...
  if (place === '広場') {
    ...
    if (rand < 0.85) {
      ...
      if (docFoundMsg) {
        docFoundMsg = withFatigueNotice(docFoundMsg);
        // 資料発見がある場合：クリック待ちで資料通知→その後アイテム取得メッセージ
        showMessage(docFoundMsg, true, () => {
          showMessage(withFatigueNotice(mainMsg), false, () => showMainActions());
        });
      } else {
        showMessage(withFatigueNotice(mainMsg), false, () => showMainActions());
      }
      ...
      if (docFoundMsg) {
        showMessage(docFoundMsg, true, () => {
          showMessage(withFatigueNotice(`${place}を探索したが、他には何も見つからなかった。`), false, () => showMainActions());
        });
      } else {
        showMessage(withFatigueNotice(`${place}を探索したが、何も見つからなかった。`), false, () => showMainActions());
      }
    }
    updateParams();
  } else {
    ...
    if (rand < battleChance) {
      ...
      _suppressLocationMsg = true;
      showMessage(withFatigueNotice(`${place}（${layer+1}層）を探索して「${msgs.join('、')}」を入手した。`), false, () => showMainActions());
      updateParams();
    } else {
      _suppressLocationMsg = true;
      showMessage(withFatigueNotice(`${place}を探索したが、何も見つからなかった。`), false, () => showMainActions());
...
function waitAction() {
  ...
  if (currentRoom !== null) {
    if (random() < 0.02) {
      ...
      passTime(1);
      _suppressLocationMsg = true;
      showMessage(withFatigueNotice('待機した。'), false, () => showLabActions());
    }
    return;
  }
  if (currentPlace === '広場' && hasCampfire) {
    ...
    passTime(1);
    _suppressLocationMsg = true;
    showMessage(withFatigueNotice('焚火で休憩した。気力と体力が回復した。'), false, () => showMainActions());
    ...
    if (random() < encounterChance) {
      ...
      passTime(1);
      _suppressLocationMsg = true;
      showMessage(withFatigueNotice('待機した。'), false, () => showMainActions());
    }
  } else {
    passTime(1);
    _suppressLocationMsg = true;
...
function passTime(hours) {
  ...
  updateElapsedTime();
  // 探索クールダウンを自動減少
  for (let place in exploreCooldown) {
    if (exploreCooldown[place] > 0) {
      exploreCooldown[place] = Math.max(0, exploreCooldown[place] - hours);
    }
  }
  // 焚き火は24時間ごとに燃料を1消費する
  if (hasCampfire && campfireFuel > 0) {
    campfireHours += hours;
    let fuelUsed = Math.floor(campfireHours / 24);
    if (fuelUsed > 0) {
      campfireHours %= 10;
      campfireFuel = Math.max(0, campfireFuel - fuelUsed);
      if (campfireFuel === 0) {
        hasCampfire = false;
        campfireHours = 0;
      }
...
function startBattleLab(room) {
  // 研究所専用エネミーをランダム選択
  let labEnemies = ['ウミウシ（変異体）', '魚（変異体）', 'タコ（変異体）'];
...
function updateBattleInfo() {
  ...
  if (overlay) {
    let monsterImage = monsterImages[battle.enemyName];
    let oldImage = leftTop.elt.querySelector('.monster-image');
    ...
    if (monsterImage) {
      ...
      image.src = monsterImage;
      image.alt = battle.enemyName;
      image.className = 'monster-image';
      let distanceScale = Math.min(1.5, Math.max(0.5, 3 / Math.max(1, battle.distance)));
      image.style.transform = `translateX(-50%) scale(${distanceScale})`;
      leftTop.elt.appendChild(image);
    }
  }
}
...
function showBattleActions() {
  ...
  // 前進（距離>1のとき有効）
  let btnFwd = createButton('⬆ 前進').attribute('data-sfx', 'none').parent(actionPanel);
  if (battle.distance <= 1) btnFwd.attribute('disabled', 'true');
  btnFwd.mousePressed(() => { playSfx('move'); playerBattleAction('forward'); });

  // 距離6のとき後退→逃走に変える
  if (battle.distance >= 6) {
    let btnEsc = createButton('🚪 逃走').attribute('data-sfx', 'none').parent(actionPanel); btnEsc.elt.style.borderColor='var(--danger)'; btnEsc.elt.style.color='#e8a090'; btnEsc
      .mousePressed(() => { playSfx('escape'); playerBattleAction('escape'); });
  } else {
    createButton('⬇ 後退').attribute('data-sfx', 'none').parent(actionPanel)
      .mousePressed(() => { playSfx('move'); playerBattleAction('backward'); });
  }
  ...
  for (let wName in weapons) {
    if ((itemCounts[wName] || 0) > 0) {
      ...
      if (battle.distance >= rMin && battle.distance <= rMax) {
        ...
        let attackButton = createButton(`${wName}（射程${rangeLabel}, 攻撃力${w.atk}）`);
        if (rMin > 1) attackButton.attribute('data-sfx', 'none');
        attackButton.parent(actionPanel)
...
function showBattleFeedMenu() {
  const healItems = ['りんご','小果実','うさぎ肉','狼の肉','干し肉','狼肉の燻製','薬草スープ','万能薬','救急キット'];
  let feedCandidates = Object.keys(itemCounts).filter(n =>
    (itemCounts[n]||0) > 0 && !healItems.includes(n) && !(n in weapons) && !unconsumableItems.has(n)
  );
  actionPanel.html('');
  textZone.html('何を食わせる？（1ターン消費・敵HP-3・敵は2ターン行動停止）');
  feedCandidates.forEach(n => {
    createButton(`${n} ×${itemCounts[n]}`).parent(actionPanel).mousePressed(() => {
      ...
      battle.enemyStopTurns = 2;
      battle.strongAttackCounter++; // 投擲で敵の行動を飛ばす分だけカウントを進める
      battle.playerTurn = true;
      updateBattleInfo();
      updateParams();
      if (battle.enemyHp <= 0) {
      ...
      showBattleMessage(`「${n}」を投げた。？？？はそれを食いついた。（HP-3）<br>敵は2ターン行動できない。`, false, () => showBattleActions());
    });
  });
  createButton('戻る').parent(actionPanel).mousePressed(() => {
    if (typeof _typeTimer !== 'undefined' && _typeTimer !== null) { clearInterval(_typeTimer); _typeTimer = null; }
...
function playerBattleAction(type, weaponName, atk) {
  ...
  if (type === 'forward') {
    ...
    playSfx(weaponName && weapons[weaponName] && weapons[weaponName].range[0] > 1
      ? 'rangedAttack' : 'select');
...
function calcEnemyAction() {
  ...
  // 投擲後は指定ターン数だけ敵の行動を止める
  if (battle.enemyStopTurns > 0) {
    battle.enemyStopTurns--;
    battle.strongAttackCounter++; // カウンターは進む
    return { msg: `？？？は動けない……（残り${battle.enemyStopTurns}ターン）`, isDead: false };
  }
