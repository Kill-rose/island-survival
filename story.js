function getKeiExpression(message) {
  if (/待って|危険|怖|傷|食べ物|10日|無理|急|気をつけ|動けない|体調/.test(message)) {
    return 'impatience';
  }
  if (/研究所|ナオト|鍵|記録|実験|真実|洞窟の奥|プロジェクト|気配/.test(message)) {
    return 'serious';
  }
  return 'default';
}

function showKeiMessage(message, waitForClick = false, afterFunc = null) {
  showPortrait('kei', getKeiExpression(message));
  showMessage(message, waitForClick, afterFunc);
}
...
function showKeiLines(lines, idx, onDone) {
  ...
  let line = lines[idx]
    .replace(/^ケイ：「([\s\S]*)」$/, 'ケイ：$1')
    .replace(/^ケイ「([\s\S]*)」$/, 'ケイ：$1');
  if (!line.startsWith('ケイ：')) line = `ケイ：${line}`;
  showKeiMessage(
    line,
    true,
...
function moveToLab(room) {
  ...
  if (room === '廊下') {
    ...
    if (!labNaotoMet && elapsedTime < 170) {
      ...
      showMessage(
        '廊下の奥で何かが動く気配がした。<br>' +
        '振り返ると——人らしきものが、こちらを向いていた。<br>' +
        'ボロボロの白衣の名札に、「林ナオト」と書かれている。<br>' +
        'しかしその顔は、もはや人の形をしていなかった。',
        true,
